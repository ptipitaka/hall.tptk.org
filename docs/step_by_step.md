# หChecklist ทีละขั้น — hall.tptk.org
...
- [x] Access key / secret — เก็บใน `.env` หรือ GitHub Secrets เท่านั้น · **อย่า paste ใน chat, issue, หรือ commit repo**
...
- [x] Scope ชัด (in / out) สอดคล้องมติ
- [x] ข้อค้างของช่วงงานปิดแล้ว หรือบันทึก defer
- [x] มี acceptance criteria 1–3 ข้อ
- [x] ทราบแหล่งข้อมูลและไม่กระทบ production โดยไม่ตั้งใจ
...
- [x] สร้างโครง Wagtail site ตาม §4 ใน development guide
- [x] `hall/settings/` — `base.py`, `dev.py`, `production.py`
- [x] Django apps: `home/`, `snippets/`, `archive/`
- [x] `frontend/` — โครง Vite multi-entry (`edition`, `master`)
- [x] `templates/`, `static/`, `scripts/`
- [x] `.gitignore` — `media/`, `.env`, ไฟล์สแกน/PDF

### 1.2 Docker Compose

- [x] `docker-compose.yml` — services `web`, `db` (PostgreSQL)
- [x] `docker compose up --build` รันผ่าน
- [x] เปิด `http://localhost:8000` ได้ (HTTP 200)

### 1.3 ตัวแปรสภาพแวดล้อม

- [x] `.env.example` ตาม development guide §3
- [x] คัดลอกเป็น `.env` บน local (ไม่ commit)
- [x] dev: `DEBUG=True`, `USE_SPACES=False`, `MEDIA_ROOT` local

### 1.4 Wagtail พื้นฐาน

- [x] ติดตั้ง Wagtail และ dependencies (`requirements.txt`)
- [x] `HomePage` ใน `home/` + `bootstrap_site` command
- [x] Wagtail admin เข้าได้ (`/admin/` HTTP 200)
- [ ] `createsuperuser` — รันครั้งแรก: `docker compose exec web python manage.py createsuperuser`

### 1.5 ตรวจสอบ Phase 1

- [x] `docker compose exec web python manage.py migrate` ผ่าน
- [x] `docker compose exec web python manage.py test` ผ่าน (4 tests)
- [x] ไม่มี secrets ใน repo
...
- [x] bucket `sacred` สร้างแล้ว — region `sgp1` — [Control Panel](https://cloud.digitalocean.com/spaces/sacred)
- [x] endpoint `AWS_S3_ENDPOINT_URL=https://sgp1.digitaloceanspaces.com` · `AWS_STORAGE_BUCKET_NAME=sacred`
